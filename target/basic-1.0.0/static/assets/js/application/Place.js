...
var Place = {
	...
	fnSrch : function() {
	    ...
	    $.ajax({
	        ...
	        success: function (workResultObj){  
				...
				var list = data.placeList;
				if ( list.length > 0 ) {
					Place.fnSetList(list);
				} else {
					$("#DivSrchList").empty(); 
				}
	        }, 
	...
	fnSetList : function(list) {
		...
		for (var i=0; i<list.length ; i++ ) {
			...
			Place.setData(target, list[i] );
		}
	},
	// 스케줄 목록 데이터 세팅
	setData : function(target, data) {
		target.attr("ondblclick", "Place.showUpdateModal('" + data.placeNo + "','" + data.title + "')"); 
		target.find("strong.title").text(data.title);          
		target.find("strong.title").attr("id", data.placeNo);    
	},
	// 스케줄 등록하기
	fnRegist : function() {
		var surl = "/rest/place/regist";
	...
	// 업데이트 모달보여주기
	showUpdateModal : function(placeNo, title) {
		$("#placeNo").val(placeNo);
	...
	fnUpdate : function() {
		...
		sendData.placeNo = $("#placeNo").val();
		sendData.dDt = sendData.dDt = $("#date").val();
		
	    $.ajax({
	        url: "/rest/place/update",
	        type: "post",
	        contentType: "application/json",  
	        data: JSON.stringify(sendData),
	        success: function (workResultObj){
